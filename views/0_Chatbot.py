...
def _maybe_copy_vectorstore():
    """
    No Streamlit Cloud o repo é read-only, então VECTORSTORE_DIR aponta para /tmp.
    Se /tmp ainda estiver vazio mas o repo tiver a base commitada, copia ela para lá.
    """
    target = config.VECTORSTORE_DIR
    repo = config.REPO_VECTORSTORE_DIR
    if target == repo:
        return  # ambiente local gravável, nada a fazer
    if target.exists() and any(target.iterdir()):
        return  # já foi copiado anteriormente
    if repo.exists() and any(repo.iterdir()):
        shutil.copytree(str(repo), str(target), dirs_exist_ok=True)


def _maybe_copy_raw_docs():
    """
    Mesmo princípio: copia data/raw/ do repo para /tmp/ebp_raw/ se necessário.
    """
    target = config.DATA_RAW_DIR
    repo = config.REPO_DATA_RAW_DIR
    if target == repo:
        return
...
init_session_state()
_maybe_copy_vectorstore()
_maybe_copy_raw_docs()
...
try:
    ...
    st.error(
        f"Erro ao carregar a base vetorial: {e}. "
        "Acesse o **Painel Administrativo** e reconstrua a base vetorial."
    )
    st.stop()
...
prompt = st.chat_input("Digite sua dúvida sobre o curso de EBP...")
